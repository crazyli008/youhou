...
(function($){
	$.fn.cxSlide=function(settings){
		...
		// 事件：序号按钮
		if(settings.btn){
			slide.numList.delegate("li",settings.events[0],function(){
		...
		// 事件：增（右）按钮
		if(settings.plus){
			slide.plusBtn.bind(settings.events[0],function(){
				slide.fn.goto();
			});
		};
		...
		// 事件：减（左）按钮
		if(settings.minus){
			slide.minusBtn.bind(settings.events[0],function(){
		...
		//
		if(settings.btn){
			slide.numList.delegate("li",settings.events[1],function(){
				slide.fn.goto(slide.numBtns.index($(this)));
			});
		};
		...
		// 事件：增（右）按钮
		if(settings.plus){
			slide.plusBtn.bind(settings.events[1],function(){
				slide.fn.goto();
			});
		};
		...
		// 事件：减（左）按钮
		if(settings.minus){
			slide.minusBtn.bind(settings.events[1],function(){
				slide.fn.goto(slide.s-1);
			});
		};
